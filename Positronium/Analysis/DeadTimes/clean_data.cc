...
void clean_data(string input_file_name, string outfile_name)
{
  ...
  acqEventPSD_t event_ch_A, event_ch_B, event_ch_C, event_ch_D;
  ...
  branch_ch_D = daq_tree->GetBranch(Form("acq_ch%i",3));
  branch_ch_D->SetAddress(&event_ch_D.timetag);
  ...
  long int nEventsD = (long int) branch_ch_D->GetEntries();
	...
	cleanEv * event_ch_D_clean = new  cleanEv;
	...
	out_tree->Branch("acq_ch3",&event_ch_D_clean);
  ...
  double acq_time=0;
	...
	while(counter_A < nEventsA and counter_B < nEventsB and counter_C < nEventsC and counter_D < nEventsD)
	{

	    // Display Progress

	    if (counter_A % (nEventsA/100) == 0 or counter_B % (nEventsB/100) == 0 or counter_C % (nEventsC/100) == 0 or counter_D % (nEventsD/100) == 0)
	    {
	        system("clear");
	    ...
	    branch_ch_D->GetEntry(counter_D);
			...
			Times[event_ch_D.timetag]=3;

			std::vector<ULong64_t> Timetag = {event_ch_A.timetag,event_ch_B.timetag,event_ch_C.timetag, event_ch_D.timetag};
			std::sort(Timetag.begin(),Timetag.end());

			ULong64_t TimeDiff=Timetag[3]-Timetag[0];
			...
			else
			{
				...
				event_ch_D_clean->timetag = event_ch_D.timetag;
				event_ch_D_clean->qlong = event_ch_D.qlong;
	...
	cout<<"coincidences "<<coincident_events<<endl<<"tot events "<<nEventsA<<"Acquisition Time [h]: "<<-1*acq_time*1E-9*4/3600<<endl;
	out_tree->Write();
	outfile->Close();
